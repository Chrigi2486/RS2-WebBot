...
class GlobalCommands(Commands):
    ...
    def servers(self, guild_id, **kwargs):
        ...
        servers = self.app.run_sql(f"SELECT SERVERS.Name, SERVERS.ServerIP, SERVERS.BMID, SERVERS.WAIP FROM SERVERS WHERE SERVERS.ID IN {tuple(self.app.active_guilds[guild_id]['servers'].values())}")
        for server in servers:
            server_name, server_IP, bmID, waIP = server
