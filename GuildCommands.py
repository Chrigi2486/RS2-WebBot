...
class GuildCommands(Commands):
    ...
    async def banid(self, data, guild_id, **kwargs):
        """Bans a given ID"""
        server = data['options'][0]['name']
    ...
    async def write(self, data, guild_id, **kwargs):
        ...
        if len(data['options'][0]['options']) == 2:
            ...
            await self.app.http.client.request(WARoute('GET', webadminip, '/current/chat/data?message={message}', message=message), cookies={'Authcred': authcred})
